...
network = tflearn.conv_2d(network, 16,3, activation='relu',regularizer="L2")
network = tflearn.max_pool_2d(network, 2)
network = tflearn.local_response_normalization(network)
network = tflearn.conv_2d(network, 32, 3, activation='relu', regularizer="L2")
network = tflearn.max_pool_2d(network, 2)
network = tflearn.local_response_normalization(network)
network = tflearn.fully_connected(network, 12, activation='tanh', regularizer="L2")
network = tflearn.dropout(network, 0.8)
network = tflearn.fully_connected(network,num_fields, activation='softmax')
network = tflearn.regression(network, optimizer='adam', learning_rate=0.0001,loss='categorical_crossentropy', name='target')

# Training
model = tflearn.DNN(network, tensorboard_verbose=0)
model.fit({'input': x_train}, {'target': y_train}, n_epoch=100,
          validation_set=({'input': x_valid}, {'target': y_valid}),
snapshot_step=100,batch_size=32, show_metric=True,shuffle=True, run_id='convnet_mnist')
